...
class TID_LQR:
    def __init__(self,A,B,Q,R,N,xos,Qos, max_iters = 1000 ):
        self.A = np.matrix(A)
        self.B = np.matrix(B)
        self.Q = np.matrix(Q)
        self.R = np.matrix(R)
    ...
    def solve_old(self):
        ...
        costs = float('inf')*np.ones((xos.shape[0]))
        stop_times = -1*np.ones((xos.shape[0]),'int')

        for t in range(self.max_iters):

            tmp = np.matrix(np.linalg.inv(R+B.T*P*B))
            K = -tmp*(B.T*P*A + N.T)
            Pn = (Q + (A+B*K).T *P*(A+B*K) )

            new_costs = np.einsum('ij,ni,nj->n',Pn-P,xos,xos) 
            new_costs += np.einsum('ii,nii->n',Pn-P,Qos)
           
            costs = np.minimum(costs, new_costs)
            stop_times[costs==new_costs] = t
        ...
        self.Ks = Ks[::-1]
        self.costs = costs
        self.start_times = self.max_iters - stop_times
    ...
    def optimal_stopping(self):
        ...
        for t in range(Ats.shape[0]-2,-1,-1):
            Ats[t,...] = np.dot(Ats[t+1,...],Afs[t,...])
        

        xfs = np.einsum('tij,nj->nti', Ats,xos)
        xcosts = np.einsum('ij,nti,ntj->nt', Q,xfs,xfs)
        Qfs = np.einsum('nij,til,tkj->ntkl', Qos, Ats, Ats)
        Qcosts = np.einsum('ntkk,kk->nt', Qfs, Q)

        costs =  xcosts + Qcosts
        #print xcosts[22,:]

        self.start_times = np.argmin(costs,1)
        self.costs = costs.min(1)
        #print self.start_times[22]
        
        #plt.scatter(xfs[22,:,1],xfs[22,:,0],c=xcosts[22,:],lw=0)
        #print xcosts[22,self.start_times[22]]
        #print xfs[22,self.start_times[22]]
        #print xcosts[22,-20:]
    ...
    def sim_all(self):
        ...
        trajs = []
        for i in range(self.start_times.size):
            trajs.append(self.sim(i) )
        return trajs
...
class CylinderMap(GaussianClusteringProblem):
    ...
    def tid_lqr_problem(self,cxd,cxos, dt = .01, max_t= 10):
        ...
        for cx,i in zip(cxos, range(len(cxos))):
            Qos[i,:-1,:-1] = cx.covariance()[:2,:2]
            xos[i,:] = np.concatenate([cx.mu[:2],np.array([1])])
        ...
        Sg = -cxd.expected_log_likelihood_matrix()
        Sg = Sg[:,(0,1,3,2)]
        Sg = Sg[(0,1,3,2),:]
        ...
        return TID_LQR(A,B,Q,R,N,xos,Qos,max_iters)


    def tid_lqr_problem_old(self, cxd, cxos, dt = .01, max_t= 10):
        ...
        clusters_plus = self.clusters+[(self.new_x_proc(), self.new_xy_proc())]
        nc = len(clusters_plus)

        A = np.zeros((nc, dx,dx))
        B = np.zeros((nc, dx,du))

        for (cx,cxy),i in zip(clusters_plus,range(nc)):
        ...
        R = np.zeros((du,du))
        N = np.zeros((dx,du))
        Q = - cxd.expected_log_likelihood_matrix()
    ...
    def plot_cluster(self,c, n = 100):
        ...
        V =  np.array(np.matrix(np.diag(np.sqrt(w)))*V)
...
class CylinderProblem():
    ...
    def adjacency_matrix(self, dt = .01):
        ...
        for s,s_ in zip(ix[:n],iy[:n]):
            ...
            As,Ks,x,u = nlqr.solve(xd_,Qd,xs_,Qs,max_iters=1)
...
class MDPtests(unittest.TestCase):
    ...
    def test_pendulum_clustering(self):
        ...
        prob = CylinderProblem(2, 10, 100, epsilon = 1e-6)
    ...
    def test_lqr3(self):
        ...
        cxd = prob.maps[md].clusters[d][0]
        cxo = prob.maps[md].clusters[o][0]


        lqr = prob.maps[md].tid_lqr_problem(cxd,[cxo], dt = dt, max_t = 5 )
        lqr.solve()
        ...
        #prob.maps[md].plot_cluster(cxd)
        #prob.maps[md].plot_cluster(cxo)
        ...
        for i in [22]:
            xs = lqr.sim(i)
            print np.einsum('ij,ni,nj->n',Q,xs,xs)
            plt.plot(xs[:,1],xs[:,0])
            #plt.scatter([cxos[i].mu[1]],[cxos[i].mu[0]])
        #print cxd.n
        
        prob.maps[md].plot_cluster(cxd)
        prob.maps[md].plot_cluster(cxos[22])

        plt.show()


if __name__ == '__main__':
    single_test = 'test_lqr3'
