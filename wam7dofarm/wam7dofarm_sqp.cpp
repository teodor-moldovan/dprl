...
#include BOOST_PP_LOCAL_ITERATE()
	...
	fill_col_major(f[T-1], INFTY*Matrix<double,X_DIM+1+6+6,1>::Ones());
	...
	fill_col_major(lb[T-1], INFTY*Matrix<double,X_DIM+1+6+6,1>::Ones());
	...
	fill_col_major(A_T, INFTY*Matrix<double,12,X_DIM+1+6+6>::Ones());
	fill_col_major(b_T, INFTY*Matrix<double,12,1>::Ones());
...
void fill_lb_and_ub(StdVectorX& X, StdVectorU& U, double& delta, double trust_box_size, bounds_t bounds)
{
	...
	VectorXd lbT_temp(X_DIM+1+6+6); lbT_temp.setZero();
...
void fill_A_T_and_b_T(StdVectorX& X, StdVectorU& U, double& delta, double trust_box_size, bounds_t bounds) {

	VectorX& x_T = X[T-1];
	Vector3d x_T_pos_eval;
	Vector3d x_T_vel_eval;
	...
	Matrix<double, 3, X_DIM> pos_jac = general_numerical_jacobian(end_effector_pos, x_T, 3);
	Matrix<double, 3, X_DIM> vel_jac = general_numerical_jacobian(end_effector_lin_vel, x_T, 3);
...
bool penalty_sqp(StdVectorX& X, StdVectorU& U, double& delta, bounds_t bounds,
	...
	while(penalty_increases < cfg::max_penalty_coeff_increases) {
		...
		if (constraint_violation <= cfg::cnt_tolerance) {
			...
			Matrix<double, X_DIM, T> init; init.setZero();
			for(int i = 0; i < X_DIM; ++i) {
...
int solve_BVP(double weights[], double pi[], double start_state[], double& delta, double virtual_control_max) {
	...
	double joint_goal[X_DIM] = { 0.        ,  0.        ,  0.        ,  0.        ,  0.       , 0.        ,  0.        ,
...
int main() {
	...
	double weights[70] = {  2.94863503e-01,   7.95022464e-03,   9.31039652e-05,   1.13500168e-01,
  ...
  -2.22110456e-04,   6.86475300e-02};
	...
	double start_state[X_DIM] = {-0.01508158,  0.01070279,  0.0741887 , -0.11166568,  0.01756522,
